...
function renderProducts(data, containerId) {
    ...
    data.forEach(item => {
        ...
        card.className = 'card'; // Matches the updated CSS class name
        
        // Error handling for missing images: shows a labeled placeholder
        const imagePath = item.img;
        
        card.innerHTML = `
            <img src="${imagePath}" alt="${item.name}" onerror="this.src='https://placeholder.com{item.id}'">
            <div class="card-body">
                <h4>${item.name}</h4>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(contactForm) {
        contactForm.addEventListener('submit', (e) => {
            e.preventDefault();
            alert('Your message has been sent to Jeff! We will respond within 48 hours.');
            contactForm.reset();
        });
    }
    ...
    if(subscribeForm) {
        subscribeForm.addEventListener('submit', (e) => {
            e.preventDefault();
            alert('Success! You are now subscribed to the Emeraldine Bambusetum list.');
            subscribeForm.reset();
        });
    }
});
