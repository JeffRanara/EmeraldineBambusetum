...
// Validation & Data Retention Logic
function saveEmailLocally(email) {
    let emails = JSON.parse(localStorage.getItem('bamboo_subscribers')) || [];
    if (!emails.includes(email)) {
        emails.push(email);
        localStorage.setItem('bamboo_subscribers', JSON.stringify(emails));
    }
}
    ...
    try {
        ...
        if (response.ok) {
            if (form.id === 'subscribe-form') {
                saveEmailLocally(data.get('email'));
                document.getElementById('newsletter-area').innerHTML = "<h3>Success!</h3><p>You've been added to our list.</p>";
            } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('contact-form').addEventListener('submit', (e) => handleFormSubmit(e, 'New Bamboo Inquiry'));
    document.getElementById('subscribe-form').addEventListener('submit', (e) => handleFormSubmit(e, 'Newsletter Signup'));
});
